...
async function renderExpertView() {
    ...
    el.innerHTML = `
        <div class="mood-card" style="background:${moodData.color}12;border-left:3px solid ${moodData.color};">
            <div class="mood-top">
                ...
                <div>
                    ...
                    <div class="mood-conf">信心度 ${moodData.confidence}</div>
                </div>
            </div>
        ...
        <div class="hl-selector">
            ${Object.keys(expertMeta).map(k => {
                const m = expertMeta[k];
                const active = k === _currentExpert ? ' active' : '';
                return `
                    <button class="hl-opt${active}" data-hl="${k}" onclick="_currentExpert='${k}';renderExpertView()">
                        <span class="hl-opt-icon">${m.icon}</span>
                        <span>${m.name}</span>
                    </button>
                `;
            }).join('')}
...
function generateExpertViews(hotNews, alerts) {
    ...
    experts.templeton = {
        insight: hasGeo ? `市场关注地缘局势变化。中东缓和的本质是风险溢价的释放——短期情绪已price in，但协议执行的波折往往被低估。逆向视角：当所有人庆祝和平时，关注被错杀的新兴市场资产。` : hasTradeWar ? `贸易摩擦的本质是全球化重构。提前布局海外产能的公司将获得双重红利。市场的线性思维低估了这个结构性变化。` : hasRate ? `全球利率周期转向。不要押注方向，而是做多波动率——同时持有股债。` : `当市场没有明确方向时，关注被短期情绪压低估值的优质资产。`,
    ...
    experts.munger = {
        insight: '三层思维拆解：「' + topTitle + '」' + (hasGeo ? '。第一层（共识）：地缘缓和利好。第二层（逆向）：协议签署日往往是高点。第三层（结构性）：中东重建需求可能持续数年。' : hasAI ? '。第一层（共识）：AI降价利好。第二层（逆向）：小公司出局。第三层（结构性）：受益的是用AI重构成本的传统行业。' : hasTradeWar ? '。第一层（共识）：关税影响出口。第二层（逆向）：供应链转移对冲。第三层（结构性）：越南印度是赢家。' : '。第一层：今天的新闻改变了什么？第二层：是否过度反应？第三层：最坏情况是什么？') + '\\n\\n检查清单：① 噪音还是信号？② 确认偏误？③ 错了最坏怎样？',
