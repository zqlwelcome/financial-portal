...
const TODAY_SCHEDULE = [
    ...
    { time: '12:00', label: '☀️ 午间', content: 'A股放量上涨，沪指涨1.5%重返3400点。成交额突破万亿，券商板块涨4.2%领涨。北向资金净流入超120亿。' },
...
function renderMasterView(el, masterId) {
    const master = WEEKLY_MASTERS[masterId];
    if (!master) return;
        ...
        <div class="weekly-list">
            ${master.views.map(v => `
                <div class="weekly-card">
                    <div class="wc-event">📌 ${v.event}</div>
                    <div class="wc-view">${v.view}</div>
                    <div class="wc-row">
                        <span class="wc-tip">💡 ${v.tip}</span>
                    </div>
                    <div class="wc-row green">
                        <span class="wc-action">⚡ ${v.action}</span>
                    </div>
                </div>
            `).join('')}
        </div>
