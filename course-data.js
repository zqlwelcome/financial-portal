  ...
  // ===== 模块四：设计篇 =====
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        content: `
        <div class="block">
          <div class="lesson-goal">🎯 本节目标：学会给AI产品设计"Plan B"，以及理解"打字机效果"怎么做好</div>
        </div>
        <div class="block">
          <h4>📖 核心知识</h4>
          <p><strong>为什么AI一定要有降级方案？</strong></p>
          <p>想象一下：你点外卖，支付页面突然白屏了。如果它直接报错"500错误"，你会骂人。但如果它说"当前支付系统繁忙，您可以稍后再试，已为您保留购物车"，你就能接受。</p>
          <p>AI也一样——它不是100%可靠的。大模型可能超时、可能答非所问、可能服务器挂了。你的工作就是：在AI不完美的时候，让用户觉得"还行"而不是"这产品真烂"。</p>
          <p><strong>四级降级体系（记住这四个等级就够了）：</strong></p>
          <p><strong>L1 - 一切正常</strong>：AI完美工作，不需要任何提示</p>
          <p><strong>L2 - AI有点怂</strong>：结果能用，但加一句话"仅供参考，请以实际情况为准"</p>
          <p><strong>L3 - AI不行了</strong>：AI完全停用，切换到人工写的固定答案或规则引擎</p>
          <p><strong>L4 - 全崩了</strong>：展示友好错误页 + 客服联系方式</p>
          <div class="tip">关键原则：降级要平滑。用户不该感觉到"AI挂了"，而是该感觉"今天推荐不太一样"。</div>
        </div>
        <div class="block">
          <h4>💼 真实案例</h4>
          <p><strong>高德地图的降级方案</strong></p>
          <p>正常→ AI实时路况+语音播报，很好用</p>
          <p>如果路况数据延迟（L2）→ 显示"5分钟前路况"，不告诉用户是延迟的</p>
          <p>如果AI路线规划不可用（L3）→ 回到最普通的静态导航，没有AI推荐了但至少还能用</p>
          <p>如果全挂了（L4）→ 展示离线地图+提示"请把车停在安全地方查看"</p>
          <p>你看，用户全程不知道"AI挂了"，只是感觉"今天导航反应慢了"。</p>
        </div>
        <div class="block">
          <h4>⚡ 流式交互：AI说话的时候你在干嘛</h4>
          <p>现在的AI会"边想边说"——你发了一条消息，它一个字一个字往外蹦，像打字机一样。这叫<strong>流式输出</strong>。</p>
          <p>这个过程有三个状态，你要给用户看不同的东西：</p>
          <p><strong>状态1：AI在思考</strong>（0-3秒）</p>
          <p>展示三个点在跳动 ···，不要转圈。超过3秒要提示"这个问题有点复杂，再等一下"。</p>
          <p>就像你跟朋友聊天，他听完问题后"嗯……"想了一会儿，你会等他。但如果他沉默了半分钟，你就要问"你还在吗？"</p>
          <p><strong>状态2：AI在说话</strong>（几秒到几十秒）</p>
          <p>逐字展示文字。关键是：内容要平滑往上滚，不要"跳一下"。</p>
          <p>如果AI在调用系统查数据（比如查你的银行余额），要展示"正在查询您的账户..."，不然用户以为卡住了。</p>
          <p><strong>状态3：说完了（或出错了）</strong></p>
          <p>正常说完 → 打字光标消失。</p>
          <p>报错 → 区分两种情况：
          <br>• "模型挂了"（服务器问题）→ "回答暂时不可用，请重试" + 重试按钮
          <br>• "结果不对劲"（AI乱说）→ "结果仅供参考，建议您核实"</p>
        </div>
        <div class="block">
          <h4>💼 案例：银行AI客服怎么做</h4>
          <p><strong>场景：</strong>用户问"我的信用卡还剩多少额度？"</p>
          <p><strong>PM要设计的完整流程：</strong></p>
          <p>第1秒 → 展示"思考中···"（告诉用户：收到了，正在处理）</p>
          <p>第2秒 → 展示"正在查询您的账户..."（告诉用户：不是乱想，是真的在查你的卡）</p>
          <p>第3-5秒 → 逐字显示"您尾号8888的信用卡，当前可用额度为3.5万元。"</p>
          <p>完成后 → 底部出现"查询账单" "申请提额"两个快捷按钮</p>
          <p><strong>如果出问题了：</strong></p>
          <p>查询API超时超过5秒 → 说"账户查询暂时繁忙"，改发"您最近一次的账单已发送到您的邮箱"</p>
          <p>大模型乱回答 → "系统检测到异常，正在转接人工客服"</p>
          <div class="tip">PM要做的重要决定：什么时候给用户看"思考过程"？银行Agent建议展示「正在查询账户...」让用户放心。闲聊Agent不需要。</div>
        </div>
        <div class="block">
          <h4>🛠️ 实操步骤（动手做一下）</h4>
          <p><strong>第1步：</strong>打开你手机里一个带AI的App（支付宝搜"AI助手"或者用ChatGPT）</p>
          <p><strong>第2步：</strong>观察它回复的效果——有没有"思考中"的动画？如果卡住了会怎么样？</p>
          <p><strong>第3步：</strong>写下你觉得做得好的3点和不好的3点</p>
          <p><strong>第4步：</strong>打开备忘录，画一个你认为更好的方案（纸笔画就行，不需要Figma）</p>
        </div>
        <div class="block">
          <h4>💡 面试攻略</h4>
          <p><strong>问题：</strong>AI功能挂了怎么办？</p>
          <p><strong>回答方向：</strong>不要说技术细节，从用户体验讲起。"首先要避免白屏报错。我的做法是分四级降级——L1正常→L2加免责声明→L3换备用方案→L4展示友好错误页。关键是要定义触发条件：比如延迟超过3秒自动降到L3。"</p>
          <p><strong>进阶问题：</strong>AI说到一半卡住了怎么处理？</p>
          <p><strong>回答方向：</strong>"要区分网络问题（展示'连接已中断，正在重连'）和AI卡壳（展示'正在思考...'）。建议设15秒超时，超时后保留已生成的内容（不要清空），让用户选择继续还是重新问。"</p>
        </div>
        <div class="block">
          <h4>📝 课后思考</h4>
          <p>设计一个AI客服的交互流程。画出来四个场景分别怎么展示：
          <br>1. 正常情况
          <br>2. API超时
          <br>3. 模型答非所问
          <br>4. 用户网络断开</p>
